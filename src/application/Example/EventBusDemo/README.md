...
/src/application/Example/EventBusDemo/
 ...
 │   │   └─ FrontDeskAggregate.ts          # place(item): Order — the only place Order is constructed
 │   └─ Application/
 │       ├─ frontDesk-store.ts             # orders[] / log[] entries — placed, ready, returned, sent again
 │       ├─ frontDesk-service.ts           # placeOrder/returnOrder — delegates construction to FrontDeskAggregate, publishing to frontDesk-publishers
 │       ├─ Event/
 │       │   ├─ frontDesk-publishers.ts     # orderPlaced/orderReturned
 │       │   └─ frontDesk-subscribers.ts    # single Boot: subscribes OrderReady + OrderSentAgain
 │       └─ FrontDeskPanel.vue
 └─ Station/
     └─ Application/
         ├─ station-store.ts              # pending[] / log[] entries carry isRepeat
         ├─ station-service.ts            # receiveOrder/receiveReturnedOrder/prepareOrder — delegates publishing to station-publishers
         ├─ Event/
         │   ├─ station-publishers.ts      # orderReady/orderSentAgain
         │   └─ station-subscribers.ts     # single Boot: subscribes OrderPlaced + OrderReturned
         └─ StationPanel.vue
```
...
FrontDesk.placeOrder(item)
  → FrontDeskAggregate.place(item)   [validates item, constructs the Order]
  → store.add(order)               [orders.unshift(...); log entry "placed"]
  → frontDeskPublisher.orderPlaced   → publish OrderPlaced [Required/Event/Domain/Example/OrderEvents]

station-subscribers (OrderPlaced)
...
Station staff clicks "Prepare" (first time — isRepeat: false)
  → station-service.prepareOrder(orderId)
    ...
    → stationPublisher.orderReady   → publish OrderReady [Required/Event/Domain/Example/OrderEvents]

frontDesk-subscribers (OrderReady)
...
FrontDesk staff clicks "Send Back" on a ready order
  → frontDesk-service.returnOrder(orderId)
    ...
    → frontDeskPublisher.orderReturned   → publish OrderReturned [Required/Event/Domain/Example/OrderEvents]

station-subscribers (OrderReturned)
...
Station staff clicks "Prepare" again (isRepeat: true)
  → station-service.prepareOrder(orderId)
    ...
    → stationPublisher.orderSentAgain   → publish OrderSentAgain [Required/Event/Domain/Example/OrderEvents — distinct from OrderReady]

frontDesk-subscribers (OrderSentAgain)
