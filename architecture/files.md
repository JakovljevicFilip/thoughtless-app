...
5. Domain Error ({Aggregate}DomainError.ts)
   1. Typed error for domain-level failures.
6. Domain Rules (Rules/{aggregate}-rules.ts)
   1. Internal domain rule helpers used only by the Aggregate.
7. Puml Schema ({Domain|Aggregate|Functionality}.puml)
...
1. Store ({aggregate}-store.ts)
   1. Application-layer Pinia store manages reactive application state.
2. Service ({aggregate}-service.ts)
   1. Application-layer CQRS orchestrator composes commands and queries and coordinates application state.
3. Command File ({aggregate}-{command}.ts)
   1. Write-side CQRS unit performs a single state-changing operation.
4. Query File ({aggregate}-{query}.ts)
...
1. Repository Implementation ({aggregate}-repository.ts)
   1. Infrastructure module performs persistence operations.
